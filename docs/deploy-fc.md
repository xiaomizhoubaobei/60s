...
| Node 版本 | 行为 |
| --- | --- |
| Node 24 | `process.features.typescript === 'strip'`，直接运行 ✅ |
| Node 22.18+ | 默认开启，直接运行 ✅ |
...
1. **基础镜像钉死 `node:22-alpine`**
   原生 `Dockerfile` 用 `node:lts-alpine`，目前是 Node 24 能跑，但 `lts` 是**浮动标签**，将来回退到 Node 20 会直接启动失败。必须钉死 `22` 或 `24`。
2. **默认 `PORT=9000`**
...
1. 基础镜像**钉死 Node 22 或 24**，禁用 `lts` 浮动标签
...
A：Node 版本过低（< 22.18 或 20 及以下）。检查基础镜像是否被浮动标签带到了 Node 20。
